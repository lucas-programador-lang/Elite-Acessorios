// ==================== THREE.JS COM SOMBRAS EM TEMPO REAL ====================
function initThreeJS() {
    ...
    // Luz direcional (principal para sombras)
    const dirLight = new THREE.DirectionalLight(0xffffff, 1.3);
    dirLight.position.set(6, 12, 8);
    dirLight.castShadow = true;

    // Configurações de qualidade das sombras
    ...
    // Luz pontual dourada
    const pointLight = new THREE.PointLight(0xd4af37, 1.4, 50);
    ...
    // === TORUS KNOT (projeta sombra) ===
    const torus = new THREE.Mesh(
        new THREE.TorusKnotGeometry(1.1, 0.32, 160, 20, 2, 3), 
        goldMat
    );
    ...
    // === ESFERAS ORBITANDO (algumas projetam sombra) ===
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (i % 2 === 0) sphere.castShadow = true; // Apenas as douradas projetam sombra
    ...
    // === PLANO QUE RECEBE SOMBRAS (sutil) ===
